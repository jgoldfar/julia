...
"""
    update!(C::Cholesky, v::StridedVector, update = true) -> CC::Cholesky

    Update or downdate a Cholesky factorization `C` with the vector `v`. If `A = C[:U]'C[:U]` then `CC = cholfact(C[:U]'C[:U] ± v*v')` but the computation `CC` only uses `O(n^2)` operations. When `update = true` an update (`+`) is computed and if `update = false` a downdate (`-`) is computed. The input factorization `C` is updated in place such that on exit `C == CC`. The vector `v` is restroyed during the computations.
"""
function update!(C::Cholesky, v::StridedVector, update = true)
    A = C.factors
    n = length(v)
    if size(C, 1) != n
...
"""
    ...
    Update or downdate a Cholesky factorization `C` with the vector `v`. If `A = C[:U]'C[:U]` then `CC = cholfact(C[:U]'C[:U] ± v*v')` but the computation `CC` only uses `O(n^2)` operations. When `update = true` an update (`+`) is computed and if `update = false` a downdate (`-`) is computed.
"""
update(C::Cholesky, v::StridedVector, update = true) = update!(copy(C), copy(v), update)
