...
struct jl_varinfo_t {
    ...
    Value *load_box; // where the current value is stored
#if JL_LLVM_VERSION >= 30700
...
{
    assert((!vi.boxroot || vi.load_box) && "undef check is null pointer for boxed things");
...
{
    if (v.typ == typ || v.typ == jl_bottom_type)
        return v; // fast-path
    Type *T = julia_type_to_llvm(typ);
    if (type_is_ghost(T))
        return ghostValue(typ);
    ...
    if (!jl_is_leaf_type(typ)) {
        ...
        if (v.TIndex) {
            ...
            Value *new_tindex = ConstantInt::get(T_int8, 0);
            SmallBitVector skip_box(1, true);
            Value *tindex = builder.CreateLoad(v.TIndex);
            ...
            if (!isa<Constant>(new_tindex)) {
                ptindex = emit_static_alloca(T_int8);
                builder.CreateStore(new_tindex, ptindex);
                if (!skip_box.all()) {
                    ...
                    if (needsroot) {
                        ...
                        if (v.V) { // oldbox might be all ghost values
                            ...
                            boxv = builder.CreateSelect(builder.CreateICmpEQ(tindex, ConstantInt::get(T_int8, 0)), emit_bitcast(oldbox, boxv->getType()), boxv);
                        }
                    ...
                    if (v.V == NULL) {
                        ...
                        return jl_cgval_t(boxv, froot, false, typ, ptindex);
                    }
                    else {
                        if (v.ispointer()) {
                            slotv = v.V;
                        ...
                        jl_cgval_t newv = jl_cgval_t(slotv, froot, false, typ, ptindex);
                        newv.tbaa = tbaa;
            ...
            if (new_idx) {
                ptindex = emit_static_alloca(T_int8);
                builder.CreateStore(ConstantInt::get(T_int8, new_idx), ptindex);
                if (v.V && !v.ispointer()) {
                    ...
                    builder.CreateStore(v.V, slotv);
                    jl_cgval_t newv = jl_cgval_t(slotv, NULL, false, typ, ptindex);
...
static jl_cgval_t emit_local(jl_value_t *slotload, jl_codectx_t *ctx)
{
    ...
    jl_cgval_t v;
    Value *isnull = NULL;
    if (vi.boxroot == NULL || vi.load_box != NULL) {
        if (!vi.isVolatile || vi.value.constant || !vi.value.V) {
            ...
            AllocaInst *tindex = NULL;
            if (vi.value.TIndex) {
                 tindex = emit_static_alloca(T_int8, ctx);
                 builder.CreateStore(builder.CreateLoad(vi.value.TIndex, /*volatile*/true), tindex);
            }
    ...
    if (vi.boxroot != NULL) {
        ...
        if (vi.load_box) {
            ...
            // as indicated by the bool in load_box
            Value *load_box = builder.CreateLoad(vi.load_box, vi.isVolatile);
            if (vi.usedUndef)
...
static void emit_assignment(jl_value_t *l, jl_value_t *r, jl_codectx_t *ctx)
{
        ...
        if (!slot.isboxed && !slot.isimmutable) {
            ...
            if (jl_is_uniontype(jt)) {
                ...
                if (slot.ispointer() && slot.V != NULL && !isa<AllocaInst>(slot.V)) {
                    isboxed = builder.CreateICmpEQ(builder.CreateLoad(slot.TIndex), ConstantInt::get(T_int8, 0));
                }
                if (dest) {
                ...
                Value *dest_tindex = NULL;
                dest_tindex = emit_static_alloca(T_int8);
                builder.CreateStore(builder.CreateLoad(slot.TIndex), dest_tindex);
                Value *gcroot = NULL;
                if (isboxed) {
                    if (slot.gcroot)
                ...
                slot = mark_julia_slot(dest, slot.typ, dest_tindex, tbaa_stack);
                slot.gcroot = gcroot;
            }
            else {
                bool isboxed;
    ...
    if (vi.boxroot) {
        // boxed variables
        if (isa<AllocaInst>(vi.boxroot))
            emit_local_root(ctx, &vi); // promote variable slot to a gcroot
        Value *rval;
        if (vi.load_box && rval_info.TIndex) {
            isboxed = builder.CreateICmpEQ(builder.CreateLoad(rval_info.TIndex), ConstantInt::get(T_int8, 0));
            builder.CreateStore(isboxed, vi.load_box, vi.isVolatile);
            ...
            if (vi.load_box) {
                assert(vi.value.TIndex);
                builder.CreateStore(ConstantInt::get(T_int1, 1), vi.load_box, vi.isVolatile);
                builder.CreateStore(ConstantInt::get(T_int8, 0), vi.value.TIndex, vi.isVolatile);
            }
    ...
    if (!vi.boxroot || (vi.load_box && rval_info.TIndex)) {
        // store unboxed variables
        if (vi.usedUndef)
            store_def_flag(vi, true);

        if (!vi.value.constant) { // check that this is not a virtual store
            assert(vi.value.ispointer() || (vi.value.TIndex && vi.value.V == NULL));
            // store tindex info
            if (vi.value.TIndex) {
                if (rval_info.TIndex) {
                    builder.CreateStore(builder.CreateLoad(rval_info.TIndex), vi.value.TIndex, vi.isVolatile);
                }
                else {
                    assert(rval_info.isboxed);
                    builder.CreateStore(compute_box_tindex(rval_info, ctx), vi.value.TIndex, vi.isVolatile);
                }
            }
            // store value
            if (vi.value.V == NULL) {
                ...
                if (rval_info.typ != vi.value.typ && !vi.value.TIndex && !rval_info.TIndex) {
